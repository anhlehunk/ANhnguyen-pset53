...
public class TodoManager extends SQLiteOpenHelper {
    ...
    // functions
    public TodoList makeList (String content){
        ...
        TodoList databaseList = new TodoList(content);
        return databaseList;
    }
    ...
    public Cursor getList () {
        // search in database
        List<TodoList> oversight = new ArrayList<>();
        String select = "SELECT * FROM " + LIST;
        SQLiteDatabase database = this.getReadableDatabase();
        Cursor listCursor = database.rawQuery(select, null);

        // set list data
        if (listCursor.moveToFirst()) {
            do {
                TodoList list = new TodoList();
                list.setId(listCursor.getInt(listCursor.getColumnIndex(_ID)));
                list.setList(listCursor.getString(listCursor.getColumnIndex(CONTENT)));
                oversight.add(list);
            } while (listCursor.moveToNext());
        }

        String[] data = new String[]{_ID, CONTENT};
        Cursor newCursor = database.query(LIST, data, null, null, null, null, null);
        if (newCursor != null) {newCursor.moveToFirst();}
        return newCursor;
    }
